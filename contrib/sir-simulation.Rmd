...
library(lubridate)
sir_df$date<-as.Date(floor_date(ymd("2020-03-30")+seconds(31*24*60*60*sir_df$time), unit="day"))
final_df <- subset(sir_df, select = -c(time))
names(final_df) <- c("susceptible", "infected", "recovered", "date")
final_df <- final_df %>% group_by(date) %>% filter(row_number(date) == max(row_number(date)))
write_csv(final_df,"daily_infections.csv")
write_csv(pivot_longer(final_df, cols = c("susceptible","infected","recovered"), names_to="infection state", values_to="population"), "daily_infections_tidy.csv")
