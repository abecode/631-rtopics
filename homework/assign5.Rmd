...
set.seed(123)
for(i in 1:50){
   samp <- sample(population, n)  # obtain a sample of size n from the population
   samp_mean[i] <- mean(samp)     # save sample mean in ith element of samp_mean
   samp_sd[i] <- sd(samp)         # save sample sd in ith element of samp_sd
}
lower <- samp_mean - 1.96 * samp_sd/sqrt(n)
...
set.seed(123)
for(i in 1:50){
samp <- sample(population, n)           # obtain a sample of size n = 15 from the population
samp_mean[i] <- mean(samp)             # save sample mean in ith element of samp_mean
samp_sd[i] <- sd(samp)                           # save sample sd in ith element of samp_sd
}
lower <- samp_mean - 2.14 * samp_sd/sqrt(n)
upper <- samp_mean + 2.14 * samp_sd/sqrt(n)
c(lower[1], upper[1])
plot_ci(lower, upper, mean(population))

```

** It looks like 6 intervals don't include the population mean **
